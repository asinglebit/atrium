...
/// atrium's own theme if one has been saved, otherwise guitar's. Falling back
/// to guitar is what keeps the two looking like one tool without either having
/// to be configured: retheme guitar and atrium follows.
/// One of the built-in themes, by the label the picker shows.
pub fn preset_named(label: &str) -> Option<Theme> {
...
fn config_path_for(tool: &str) -> PathBuf {
